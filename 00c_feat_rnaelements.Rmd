...
subtitle: "input: nanostring elements files & demographics; output: well formatted data and meta col files"
output:
  html_document:
    theme: paper
    highlight: zenburn
...
libr(append(pkgs(), c("pracma","NormqPCR","mixOmics","amritr")))
```


```{r}
load(rnaseq_to_ns_dir) # load list of biomarker candidates

# 1) HBA attenutation (Discovery cohort)

setName <- paste("Set", 1:length(hba_attenuation_discovery_dirs))
date <- c("2016-01-14","2016-01-16","2016-01-16", "2016-01-16") #date?
cartridgeLotNum <- rep("31020635468_exp02.2016", length(hba_attenuation_discovery_dirs))
Tag156LotNum <- rep("TS2877-1", length(hba_attenuation_discovery_dirs))
ext24TagLotNum <- rep("TS4006", length(hba_attenuation_discovery_dirs))
plateLotNum <- rep("41000274-0056_exp02.2016", length(hba_attenuation_discovery_dirs))
DatList <- list()
for(h in 1 : length(hba_attenuation_discovery_dirs)){
  files <- list.files(hba_attenuation_discovery_dirs[h], full.names = TRUE)
...
write.csv(Dat, hba_attenuation_discovery_dir)

#------------
# import data ### NO METADATA!!
#------------
sampleMap0 <- read.csv(hba_attenuation_discovery_demo_dir)
...
data <- read.csv(hba_attenuation_discovery_data_dir, row.names = 1)
...
#----------------------------------------------
setName <- paste("Set", 1:length(hba_attenuation_confirmatory_data_dirs))
date <- rep(c("2016-02-10","2016-02-11"), each = 3)
cartridgeLotNum <- rep("31020635467_exp12.2016", length(hba_attenuation_confirmatory_data_dirs))
Tag156LotNum <- rep("TS2877-1", length(hba_attenuation_confirmatory_data_dirs))
ext24TagLotNum <- rep("TS4006", length(hba_attenuation_confirmatory_data_dirs))
plateLotNum <- rep("41000274-0056_exp02.2016", length(hba_attenuation_confirmatory_data_dirs))
DatList <- list()
for(h in 1:length(hba_attenuation_confirmatory_data_dirs)){
  files <- list.files(hba_attenuation_confirmatory_data_dirs[h], full.names = TRUE)
...
Dat <- do.call(rbind, DatList)
write.csv(Dat, hba_attenuation_confirmatory_data_dir)

#------------
# import data ### NO MAPPING FILE
#------------
mappingFile <- read.csv(hba_attenuation_confirmatory_demo_dir, row.names = 7)
data <- read.csv(hba_attenuation_confirmatory_data_dir, row.names = 1)
...
```{r}
demo0 <- readRDS(meta_file_rnapc_dir)
...
## Positive Control Normalization
posNF <- posDat %>% group_by(fileName) %>% 
  dplyr::summarise(posGeoMean = geometric.mean(Count)) %>% 
...
save(m, file=paste0(feat_rnaelements_dir,".pre.Rdata"))
write.csv(m, file=paste0(feat_rnaelements_dir,".pre.csv"))
...
save(gene_dt, file=paste0(meta_col_rnaelements_dir,".Rdata"))
write.csv(gene_dt, file=paste0(meta_col_rnaelements_dir,".csv"))
```
