...
# helpers
source('code/mixOmics_plotting_helpers.r')
```
...
X <- list(
  # 'Imputed cells'      = cells,
  'Flow cytometry'     = readRDS('data/hipc_flow_concs.rds'),
  # 'Flow cytometry'     = readRDS('data/hipc_flow_concs.rds') %>% asinh(),
  # 'Flow cytometry'     = readRDS('data/hipc_flow_autoprops_concs.rds'),
  'Luminex cytokine'   = readRDS('data/hipc_cytokine_concs.rds'),
  'Metabolomics'       = readRDS('data/hipc_metabolomics_concs.rds'),
  # 'Metabolomics'       = readRDS('data/hipc_metabolomics_eigens.rds'),
  'Proteomics'         = readRDS('data/hipc_protein_ratios_updated.rds'),
  # 'Transcriptomics'    = readRDS('data/hipc_hancock_counts_sva_resequenced.rds')
  'Transcriptomics'    = readRDS('data/hipc_hancock_counts_resequenced.rds')
)
...
```{r indexed_vs_nonindexed, include=FALSE, cache=TRUE, eval=FALSE}
...
```{r cleanup_subset, include=FALSE, cache=TRUE}
...
It is clear that the development of the immune system over the first week of life is a major source of variation in most of the data blocks, though some days are more difficult to separate than others.
...
```{r kfold, cache=TRUE, include=FALSE}
# cross-validation
set.seed(165)
# set.seed(199)
...
```{r extract_features, include=FALSE}
# get model features
...
```{r consensus_plot, include=FALSE}
...
```{r single_omics_models, include=FALSE}
...
The first components across the data blocks are all highly correlated to each other, and our response (days of life), by design. Each component can be thought of as a functional unit maximally correlated to the response. As such, comparing their membership can be insightful.
...
temp$name <- NA # upset can't highlight individual bars otherwise ?!?
upset(
  temp,
  nintersects = NA,
