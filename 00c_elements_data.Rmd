...
libr(c("dplyr","dplyr","tidyr",
       "psych",  # geometric.mean()
       "NormqPCR", # selectHKs
       "ggplot2",
       "amritr",# devtools::install_github("singha53/amritr")
       "mixOmics"))
...
mappingFile <- read.csv(paste0(data_dir, "/data/validation/HBA2_attenuation_replication/validationCohort_mappngFile.csv"), row.names = 7)
data <- read.csv(paste0(data_dir, "/data/validation/HBA2_attenuation_replication/HBA2Attenutation_asthmaBiomarkersValidation.csv"), row.names = 1)
...
## FLIPPERS --------------------------------------
response_table = rowSums(table(as.character(demo0$NAME), demo0$calculated_Response) != 0)
flippers <- names(response_table[response_table >1])
flippers_non <- names(response_table[response_table ==1])
flippers_non = flippers_non[!grepl("Normal",flippers_non)]
demo1_val_repeat <- demo0[demo0$NAME %in% flippers, ]
table(as.character(factor(demo1_val_repeat$NAME)), demo1_val_repeat$calculated_Response)
# remove subjects challenges with different allergens
demo1_val_repeatDifferentResponse <- demo1_val_repeat[!(as.character(demo1_val_repeat$NAME) %in% names(rowSums(table(as.character(factor(demo1_val_repeat$NAME)), as.character(factor(demo1_val_repeat$Allergen_cleanLabel))) != 0)[rowSums(table(as.character(factor(demo1_val_repeat$NAME)), as.character(factor(demo1_val_repeat$Allergen_cleanLabel))) != 0) == 2])), ]

## same response
demo_sameResponse1 <- subset(demo0, calculated_Response != "Control")
dropFlippers <- rownames(table(as.character(factor(demo_sameResponse1$NAME)), demo_sameResponse1$calculated_Response))[rowSums(table(as.character(factor(demo_sameResponse1$NAME)), demo_sameResponse1$calculated_Response) == 0) == 1]
...
```{r}
save(demo1_dis=demo1_dis, demo1_val=demo1_val, 
     y.train=y.train, y.test=y.test,
     pos_hk_demo1_disList=pos_hk_demo1_disList, 
     pos_hk_demo1_valList=pos_hk_demo1_valList,
