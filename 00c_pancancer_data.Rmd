...
knitr::opts_chunk$set(tidy = TRUE, echo = T, cache = FALSE, warning = FALSE, message = FALSE)
...
ann0 <- read.csv(paste0(root, "/data/Cells_nCounter_Human_PanCancer_Immune_Profiling_Panel_Gene_List.csv"), row.names = 1)
table(ann0$Gene.Class)

ann <- ann0[rownames(genEset), ]

## plot total number of genes
table(as.character(ann$Gene.Class))[order(table(as.character(ann$Gene.Class)))] %>% 
  as.data.frame %>% 
