...
library(ggbiplot); #ggbiplot() #devtools::install_github("vqv/ggbiplot")
...
```{r}
filePaths <- c(paste0(data_dir, "/data/rawdata/nanoString/20150703_asingh - pancancer MAXset A_RCC"),
               paste0(data_dir, "/data/rawdata/nanoString/20150709_setB max fov - ykw n amrit_RCC"),
               paste0(data_dir, "/data/rawdata/nanoString/20150709_Set3-C-AMRIT-KYW-MAX_RCC"))
...
write.csv(Dat, paste0(data_dir, "/data/preprocessedData/cleanedRawData_panCancerDiscovery.csv"))

#------------
# import data 
#------------
sampleMap <- c(paste0("E", 1:36))
data <- read.csv(paste0(data_dir, "/data/preprocessedData/cleanedRawData_panCancerDiscovery.csv"), row.names = 1)
names(sampleMap) <- unique(data$fileName)
data$Sample <- sampleMap[as.character(data$fileName)]
write.csv(data, paste0(data_dir, "/data/preprocessedData/cleanedRawData_panCancerDiscovery.csv"))


## Demographics
demo <- readRDS(paste0(root, "/data/RNAseq/demo/allsitesDemo.rds"))
...
pdf(paste0(data_dir, "/results/Figures/Fig.S1.pdf"), height = 8, width = 8)
...
totalNF <- dataNormEndoFiltered %>%
  group_by(Sample.x) %>% 
  dplyr::summarise(totalsumMean = sum(posNorm)) %>%
...
ann0 <- read.csv(paste0(root, "/data/geneAnnotations/Cells_nCounter_Human_PanCancer_Immune_Profiling_Panel_Gene_List.csv"), row.names = 1)
...
save(demoPre=demoPre, genEset=genEset,
     ...
     file = paste0(data_dir, "/data/preprocessedData/panCancerDatasets.RDATA"))
```
