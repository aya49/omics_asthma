...
## load dataset
load(paste0(WhereAmI, "data/preprocessedData/panCancerDatasets.RDATA"))
...
#pdf(paste0(WhereAmI, "results/Figures/Figure1A.pdf"), height = 5)
p1
#dev.off()
...
ais <- demoPre$AIS
ais[is.na(ais)] <- -1
#pdf(paste0(WhereAmI, "results/Figures/Figure1B.pdf"), height = 5, width = 5)
...
#saveRDS(result, paste0(WhereAmI, "results/latePhaseBiomarkerPanel.rds"))
result <- readRDS(paste0(WhereAmI, "results/latePhaseBiomarkerPanel.rds"))
biomarkerPanel <- result$panel
#write.csv(biomarkerPanel, paste0(WhereAmI, "results/biomarkerPanelgenes.csv"))
selectedGenes <- genEset[biomarkerPanel, ]

## Determine classificatoin performance
#cv <- perf.tuned.sPLSDA(result)
#saveRDS(cv, paste0(WhereAmI, "results/panelPerformance.rds"))
cv <- readRDS(paste0(WhereAmI, "results/panelPerformance.rds"))
...
#saveRDS(mrnaPerf.immRes, paste0(WhereAmI, "results/panelLOOCV.rds"))
mrnaPerf.immRes <- readRDS(paste0(WhereAmI, "results/panelLOOCV.rds"))
...
#pdf(paste0(WhereAmI, "results/Figures/Figure2A.pdf"), height = 8, width = 8)
...
#pdf(paste0(WhereAmI, "results/Figures/Figure2B.pdf"), height = 6, width = 6)
...
#pdf(paste0(WhereAmI, "results/Figures/Figure3A.pdf"), height = 8, width = 8)
...
#pdf(paste0(WhereAmI, "results/Figures/Figure3B.pdf"), height = 8, width = 8)
...
nrow(cdMol)
#write.csv(cdMol, paste0(WhereAmI, "results/enrichr/biomarkerpanel/cdMoleculeslist.csv"))
## run enrichr on those list of genes (Human Gene Atlas results)
cdMol_humanGeneAtlas <- read.delim(paste0(WhereAmI, "results/enrichr/biomarkerpanel/cdMolecules_Human_Gene_Atlas_table.txt"))
head(cdMol_humanGeneAtlas)

innImmuneGenes <- filter(dfc2, Pathway == "Innate immune response")
nrow(innImmuneGenes)
#write.csv(innImmuneGenes, paste0(WhereAmI, "results/enrichr/biomarkerpanel/innImmuneGenesList.csv"))
## run enrichr on those list of genes (Human Gene Atlas results)
innImmuneGenes_wikiPathways <- read.delim(paste0(WhereAmI, "results/enrichr/biomarkerpanel/innImmuneGenes_WikiPathways_2016_table.txt"))
...
#saveRDS(dingoResult, paste0(WhereAmI, "results/dingoResult.rds"))

cutOff = 2.5
fit.dingo <- readRDS(paste0(WhereAmI, "results/dingoResult.rds"))
...
#pdf(paste0(WhereAmI, "results/Figures/Figure4.pdf"), height = 10, width = 10)
...
```{r}
genesetEnrich <- read.delim(paste0(WhereAmI, "results/enrichr/asthmaExacerbationGenes/Reactome_2016_table.txt"))
...
```{r}
lincsChemDown <- read.delim(paste0(WhereAmI, "results/enrichr/asthmaExacerbationGenes/LINCS_L1000_Chem_Pert_down_table.txt"))
...
```{r}
lincsLigandUp <- read.delim(paste0(WhereAmI, "results/enrichr/asthmaExacerbationGenes/LINCS_L1000_Ligand_Perturbations_up_table.txt"))
...
#pdf(paste0(WhereAmI, "results/Figures/Figure6.pdf"), height = 7, width = 11)
