...
## input directory
data_dir = paste0(root, "/data/RNAseq_nanostring")
...
## libraries
source("code/_func.R")
source("code/_func-classifiers.R")
libr("tidyverse")
libr("limma")
libr("igraph")
libr("ggrepel")
...
load(paste0(data_dir, "/data/rnaseqDatasets.RDATA"))
...
```{r}
# cell=non/leucocytes demo; make response a factor, choose 7 columns, except response collate all cell types into a col and their freq into a col
...
plot(fit, main="Leukocytes ~ response")
...
plot(fit, main="neutrophils ~ response")
shapiro.test(fit$residuals) # null = not normal distri
qqnorm(fit$residuals); qqline(fit$residuals, col = 2)
summary(fit) # Pt=0.19

# Lymphocytes
fit <- lm(rnaseq_demo_pre$lym_percent ~ rnaseq_demo_pre$calculated_Response)
plot(fit, main="lymph ~ response")
shapiro.test(fit$residuals) # normal
qqnorm(fit$residuals); qqline(fit$residuals, col = 2)
summary(fit) # Pt=0.70
...
### non/normally distributed demo only
...
rnaseq_demo_pre %>% dplyr::select(HT.cm., BLFEV, LAR, AIS, calculated_Response) %>% 
  gather(DEMO, freq, -calculated_Response) %>% 
...
```{r}
# genes ~ response
design_resp <- model.matrix(~factor(rnaseq_demo_pre$calculated_Response, levels = c("ER", "DR")))
...
design_resp.leuk <- model.matrix(~factor(rnaseq_demo_pre$calculated_Response, levels = c("ER", "DR"))
                                 + Leukocyte_Counts.x10.9., data = rnaseq_demo_pre)
nrow(rnaseq_demo_pre[rownames(design_resp.leuk), ]); table(rnaseq_demo_pre[rownames(design_resp.leuk), "calculated_Response"])
...
```{r}
# gene counts ~ response
design_resp <- model.matrix(~factor(rnaseq_demo_post$calculated_Response, levels = c("ER", "DR")))
...
design_respDat <- data.frame(response = rnaseq_demo_post$calculated_Response,
                             cc = rnaseq_demo_post[, "Leukocyte_Counts.x10.9."] - rnaseq_demo_pre[, "Leukocyte_Counts.x10.9."])
rownames(design_respDat) <- rownames(rnaseq_demo_post)
nrow(rnaseq_demo_post[rownames(na.omit(design_respDat)),])
table(rnaseq_demo_post[rownames(na.omit(design_respDat)), "calculated_Response"])
...
mapID_genSym <- mapply(
  function(x, y) { annotateTranscripts(features=rownames(x), filter = y, mart=mart, attr=attr) }, 
  x = deg_pre_cc_common, 
  y = list("ucsc", "ucsc", "ensembl_gene_id", "trinity")
)
sigGenes <- unique(unlist(mapID_genSym))
length(sigGenes);
write.csv(sigGenes, paste0(data_dir, "/result/diff_Exp/significantgenes_DEanalysis.csv"))
...
design_resp <- model.matrix(~factor(rnaseq_demo_pre$calculated_Response, c("ER", "DR")))
