...
## output: string function-ed gene etc maps
## aya43@sfu.ca
## created 20180524
...
cid_col = "probe"
cinclud_col = c("dbSNP","chromosome","Associated Gene","EBI MAPPED GENE(S)","ClinVar OMIM Gene", "ClinVar OMIM Description")
key_word = "asthma"
...
key_row = apply(meta_col0,1,function(x) any(grepl(key_word,x)))
key_col = apply(meta_col0,2,function(x) any(grepl(key_word,x)))

meta_col = meta_col0[key_row,c(cinclud_col,colnames(meta_col)[key_col])]
...
lapply(associated_gene, function(ag) {
...
c("transcript_accession", "SNP-gene_relation", 
