...
source(paste0(root, "/code/crossValidationPerf.R"))
libr(append(pkgs(),c("mixOmics","tidyverse","igraph",
                     "ggrepel","biomaRt","sear","RColorBrewer")))
```
...
load(rnaseqa_datanew_dir) #rnaseq_meta_file
# rnaseq_meta_file = rnaseq_demo
# rnaseq_meta_file = rnaseq_meta_file[order(rownames(rnaseq_meta_file)),]

# rnaseq_datasets = lapply(rnaseq_datasets, function(x) x[,order(colnames(x))])

# time=Pre meta_file and data
rnaseq_meta_file_pre <- subset(rnaseq_meta_file, Time == "Pre")
rnaseq_datasets_pre <- lapply(rnaseq_datasets, function(i) i[,rownames(rnaseq_meta_file_pre)] )

# time=Post meta_file and data
rnaseq_meta_file_post <- subset(rnaseq_meta_file, Time == "Post")
rnaseq_datasets_post <- lapply(rnaseq_datasets, function(i) i[,rownames(rnaseq_meta_file_post)] )
...
pdf(paste0(rnaseq_dir, "/result/Figure2a.pdf"), width = 15, height = 5)
...
pdf(rnaseq_2b_dir, width = 15, height = 5)
...
trinity_map_dir = trinity_map_dir
...
length(sigGenes);
write.csv(sigGenes, rnaseq_sig_dir)
...
cbPalette <- c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7",
               brewer.pal(9, "Set1"))
pdf(rnaseq_2c_dir, width = 8, height = 5.5)
...
```{r}
biocartaPathways <- read.delim(results/enrichr_difbc_dir)
biocartaPathways <- subset(biocartaPathways, Adjusted.P.value < 0.15)
keggPathways <- read.delim(results/enrichr_difkegg_dir)
keggPathways <- subset(keggPathways, Adjusted.P.value < 0.15)
wikiPathways <- read.delim(results/enrichr_erwiki_dir)
...
pdf(rnaseq_2d_dir, width = 10, height = 10)
...
folds.iter <- lapply(readRDS(rnapc_preeperfcov_dir), function(i) i$folds.iter) 
...
#delta
#saveRDS(cv, paste0(rnaseq_dir, "/results/rnaseq_biomarkerPanelPerformanceResults.rds"))

cv <- readRDS(rnapc_panelperfresult_dir)
...
pdf(rnaseq_3a_dir, width = 6, height = 4)
...
#saveRDS(FreqPanels, paste0(rnaseq_dir, "/results/biomarkerCandidateSelection.rds"))

FreqPanels <- readRDS(paste0(rnaseq_dir, "/results/biomarkerCandidateSelection.rds"))
...
grid.newpage();
pdf(paste0(rnaseq_dir, "/results/Figures/Figure3/Figure3b.pdf"), width = 5, height = 5)
grid.draw(venn.plot);
dev.off()
...
for(i in 1 : length(hk)){
  ...
  if(filter == "trinity"){
    trinityMapFile <- read.delim(rnaseqa_trinbl_dir)
    trinityMapFile$Contig <- unlist(lapply(strsplit(as.character(trinityMapFile$query_id), "_"), function(i) paste(i[1], i[2], sep="_")))
    trinityMapFile$UniProt <- unlist(lapply(strsplit(unlist(lapply(strsplit(as.character(trinityMapFile$subject_id), "\\|"), function(i) i[[2]])), split="_"), function(x) x[1]))
    trinityMapFile$GenSym <- unlist(lapply(strsplit(unlist(lapply(strsplit(as.character(trinityMapFile$subject_id), "\\|"), function(i) i[[3]])), split="_"), function(x) x[1]))
    gene <- unlist(lapply(1 : length(features), function(z){
...
pdf(rnaseq_3c_dir, width = 6, height = 4)
...
pdf(rnaseq_3d_dir, width = 6, height = 4)
