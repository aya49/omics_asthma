##################################################
#  GGR_validation_collect_stats.R
#
#  $proj/Scripts/causality/GGR/gtex/GGR_validation_collect_stats.R
# 
...
# original_dir = paste0('/tigress/BEE/RNAseq/Output/causality/GGR/output/best_cis_only/', condition, '/')
# regression = 'enet-2'
# write_dir = paste0('/tigress/BEE/RNAseq/Output/causality/GGR/final_list/best_cis_only/', condition, '/')
# graph_dir = paste0('/tigress/BEE/RNAseq/Analysis/Figures/GGR_validation_plots/best_cis_only/', condition, '/')
...
for (tissue in tissues) {
	print(tissue)
	# which files carry the information about the tissue of interest?
	# ind = sapply(list_files, function(x) {strsplit(strsplit(x, '_')[[1]][8], '\\.')[[1]][1]}) == tissue
	...
	# cumulative_me_trans_total = vector('list', num_parts)
	# cumulative_me_trans_permute_total = vector('list', num_parts)
	# MR_full_table_total = vector('list', num_parts)
	...
	for (i in c(1:num_parts)) {
		...
		if (file.exists(file)) {
			...
			full_master_table[[i]] = master_table
		} else {
	...
	full_master_table = full_master_table[(full_master_table$snp != ''),]
	full_master_table = full_master_table[order(full_master_table$trans_pval),]

	# Uncomment if qq plot is wanted
	qq_plot_df = data.frame(true = full_master_table$trans_pval, null = full_master_table$trans_permute_pval[order(full_master_table$trans_permute_pval)], tissue = tissue, stringsAsFactors=FALSE)
	...
	qobj = qvalue(full_master_table$trans_pval)
	full_master_table$FDR = qobj$qvalue
	print(qobj$pi0)

	below_0.1[tissue] = sum(full_master_table$FDR<0.1)
	ntests[tissue] = all_n_tests_trans_total
	pi_zeros[tissue] = qobj$pi0

	# save the final table
	write.table(full_master_table[full_master_table$trans_pval < 0.01,], file=paste0(write_dir, tissue, "_", regression, "_trans_eQTL_table.txt"), sep='\t', quote=F, row.names=F)
}
...
summary_table = cbind(do.call('rbind', below_0.1), do.call('rbind', ntests))
summary_table = cbind(summary_table, do.call('rbind', pi_zeros))
colnames(summary_table) = c('n_below_0.1', 'ntests', 'pi_zeros')
print(summary_table)

a = summary_table['lung','n_below_0.1']
b = as.numeric(colSums(summary_table)['n_below_0.1'] - a)
c = summary_table['lung','ntests']
d = as.numeric(colSums(summary_table)['ntests'] - c)
