##################################################
#  gwas_variants_bayes_freq_MR_stats.R
#
#  $proj/Scripts/causality/bayes_MR/gwas_variants_bayes_freq_MR_stats.R
# 
...
# Example
# args = c(1:5)
# args[1] = '/tigress/BEE/RNAseq/Data/Resources/gwas/gwas-association-downloaded-thyroid.tsv'
# args[2] = '/tigress/BEE/RNAseq/Data/Expression/gtex/hg38/GTEx_Analysis_v8_eQTL_expression_matrices/Thyroid.v8.normalized_expression.bed.gz'
# args[3] = 'Thyroid'
...
gwas_table = read.table(gwas_table_location, header=T, stringsAsFactors=F, sep='\t', fill=T)
# only going to consider autosomal
gwas_table = gwas_table[!is.na(as.numeric(gwas_table$CHR_ID)),]
gwas_table = gwas_table[!is.na(as.numeric(gwas_table$CHR_POS)),]
gwas_table = gwas_table[order(gwas_table$CHR_ID),]
...
# Repeat for each chromosome - but the logic is essentially the same with wabf_calculate_bayes_factor_MR_stats.R
for (chr_number in c(1:22)) {

# Read in the genotype matrix, and see which ones are in our GWAS catalogue
gwas_table_subset = gwas_table[gwas_table$CHR_ID == chr_number,]
genotype_matrix_master_05 = read.table(paste0(proj_dir, '/Data/Genotype/gtex/v8/allelic_dosage/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_838Indiv_chr',chr_number,'_dosage_MAF_05.txt'), header = TRUE, sep = '\t', stringsAsFactors=FALSE)
locs = as.numeric(sapply(genotype_matrix_master_05$X, function(x) {strsplit(x, '_')[[1]][2]}))
locs_inds = sapply(locs, function(x) {x %in% gwas_table_subset$CHR_POS})
genotype_matrix_master = genotype_matrix_master_05[locs_inds,]

genotype_matrix_master_01 = read.table(paste0(proj_dir, '/Data/Genotype/gtex/v8/allelic_dosage/GTEx_Analysis_2017-06-05_v8_WholeGenomeSeq_838Indiv_chr',chr_number,'_dosage_MAF_01.txt'), header = TRUE, sep = '\t', stringsAsFactors=FALSE)
locs = as.numeric(sapply(genotype_matrix_master_01$X, function(x) {strsplit(x, '_')[[1]][2]}))
locs_inds = sapply(locs, function(x) {x %in% gwas_table_subset$CHR_POS})
genotype_matrix_master = rbind(genotype_matrix_master, genotype_matrix_master_01[locs_inds,])

# Make sure the columns are the same
genotype_matrix = genotype_matrix_master[,c(2:ncol(genotype_matrix_master))]
rownames(genotype_matrix) = genotype_matrix_master$X
colnames(genotype_matrix) = sapply(colnames(genotype_matrix), function(x) {paste(strsplit(x, '\\.')[[1]][1], strsplit(x, '\\.')[[1]][2], sep = '-')})
...
if (nrow(genotype_matrix) == 0) {next}
...
for (i in c(1:nrow(genotype_matrix))) {
	...
	eqtl_candidates = calc_WABF(exp_all_genes, temp_cov, W, PO, 0.05)
    ...
    abf_eqtl_list[[cis_counter]] = eqtl_candidates

    # Mark the gene locations
	cis_eqtl_list = abf_eqtl_list[[cis_counter]]
	cis_eqtl_list = cis_eqtl_list[cis_eqtl_list$gene_chr == paste0('chr', chr_number),]
	cis_counter = cis_counter + 1
	...
	for (j in c(1:nrow(cis_eqtl_list))) {
		...
		MR_stats_list[[trans_counter]] = total_df
...
save(eqtl_out_df, mr_out_df, file = paste0(out_file, '_cis_risk_', cis_risk, '_pi1_', pi_1,  '_trans_risk1_', trans_risk_1, '_trans_risk2_', trans_risk_2, '_snp_effect_', snp_pi_1, '_gene_effect_', gene_pi_1, '.RData'))
