...
#  wabf_functions.R
#
#  $proj/Scripts/causality/bayes_MR/wabf_functions.R
# 
...
# Helper functions
calc_WABF_with_NAs = function(exp, gene_list, genotype, cov, W, PO) {
	# Remove rows from genotypes that are NA
	inds = !is.na(genotype)
	# Return if genotype is uniform
	if (length(unique(as.numeric(genotype[,inds]))) == 1) {return}
	# Get the SNP MAF
	...
	r = W/(V+W)
	# Z-statistic
	Z = betas[gene_list] / sqrt(V)
	# Approximate Bayes Factor
	ABF = (1/sqrt(1-r)) * exp(-r*(Z^2)/2)
	# Posterior probability of association
	PPA = 1 / (ABF*PO + 1)

	return_frame = data.frame(row.names = NULL, snp = rownames(genotype), gene = gene_list, beta = betas[gene_list], stat = Z, abf = ABF, ppa = PPA, maf = snp_maf, snp_var_exp = var_exp)
	return(return_frame)
}
