...
# args[6] = '/tigress/BEE/RNAseq/Output/trans-mapping/gtex/MatrixEQTL/all-by-all/muscleskeletal/muscleskeletal_nonverlapping_certain_autosomes_normalized_MatrixEQTL'
# args[7] = '10'
...
part_number = args[2]
chr_number = ceiling(as.numeric(part_number)/10)
...
# if(method == 'gtex_gct_v6p') {
  ...
  ## Added for this particular job
  if ('gender' %in% rownames(covars)) {
    restricted_covars = covars[c('C1', 'C2', 'C3', 'gender', 'Platform'),]
    num_to_take = round(2*(dim(covars)[1]-5)/3)
  } else {
    restricted_covars = covars[c('C1', 'C2', 'C3', 'Platform'),]
    num_to_take = round(2*(dim(covars)[1]-4)/3)
  }
  restricted_covars = rbind(restricted_covars, covars[c(4:(4 + num_to_take - 1)),])
  print(dim(restricted_covars))
...
cvrt = SlicedData$new()
cvrt$CreateFromMatrix(as.matrix(restricted_covars));
...
save(cumulative_me_cis, cumulative_me_trans, cumulative_me_cis_permute, cumulative_me_trans_permute, all_n_tests_cis, all_n_tests_trans, all_hist_vals_cis, all_hist_vals_trans, all_n_tests_cis_permute, all_n_tests_trans_permute, all_hist_vals_cis_permute, all_hist_vals_trans_permute, gene_set_size, snp_set_size_init, snp_set_size_final, file=paste(out_file, '_part', part_number, '_me.RData', sep="") )
