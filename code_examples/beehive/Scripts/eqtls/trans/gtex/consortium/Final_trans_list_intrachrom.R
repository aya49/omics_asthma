##################################################
#  Final_trans_list_intrachrom.R
#
#  $proj/Scripts/eqtls/trans/gtex/consortium/Final_trans_list_intrachrom.R
# 
...
# in_dir = paste0(proj_dir, '/Output/trans-mapping/gtex/trans-eQTLs/MatrixEQTL/intrachromosomal/')
# dist_thresh = 1e5
...
list_files = list.files(path=paste0(in_dir, 'eqtl_list/'), pattern='_list_by_pvalue.RData')
...
for (file in list_files) {
	...
	if (sum(cumulative_me_trans_total$FDR <= 0.5) > 0) {
		...
		write.table(total_trans_df, file=paste0(in_dir, 'eqtl_list_post_crossmapping/', paste(tissue_name, 'trans_list_FDR_0.5.txt', sep='_')), quote = FALSE, sep = "\t", row.names = FALSE)
		if (nrow(total_trans_df) > 0) {
			total_trans_df$tissue = tissue_name
			all_tissue_total_trans_df[[tissue_name]] = total_trans_df
		}
	} else {
		write.table(null_df, file=paste0(in_dir, 'eqtl_list_post_crossmapping/', paste(tissue_name, 'trans_list_FDR_0.5.txt', sep='_')), quote = FALSE, sep = "\t", row.names = FALSE)
	}
...
write.table(all_tissue_total_trans_df, file=paste0(in_dir, 'trans-eQTLs_FDR-0.5.txt'), quote = FALSE, sep = "\t", row.names = FALSE)
write.table(all_tissue_total_trans_df[all_tissue_total_trans_df$FDR < 0.1,], file=paste0(in_dir, 'trans-eQTLs_FDR-0.1.txt'), quote = FALSE, sep = "\t", row.names = FALSE)
