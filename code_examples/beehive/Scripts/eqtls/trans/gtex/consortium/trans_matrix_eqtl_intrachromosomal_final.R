##################################################
#  trans_matrix_eqtl_intrachromosomal_final.R
#
#  $proj/Scripts/eqtls/trans/gtex/consortium/trans_matrix_eqtl_intrachromosomal_final.R
# 
#  This version performs the all-by-all cis-mapping analysis for intrachromosomal analysis.
#
#  Author: Brian Jo
#
##################################################
...
# Take only the cis-genes:
ind = (gene_positions$chr == snp_positions$chr[1])
expression_matrix = expression_matrix[ind,]
gene_positions = gene_positions[ind,]
gene_set_size = dim(expression_matrix)[1]

me = MatrixEQTL_wrapper(genotype_matrix, expression_matrix, snp_positions, gene_positions, pvThresh = pvOutputThreshold, pvThresh_cis = pvOutputThreshold, cis_dist = cis_dist, covariates = covars)
...
save(me, gene_set_size, snp_set_size_init, snp_set_size_final, file=paste0(out_file, '_part', sprintf("%03d", part_number), '.RData') )
