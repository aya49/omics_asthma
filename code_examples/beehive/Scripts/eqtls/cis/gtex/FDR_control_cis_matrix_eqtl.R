##################################################
#  FDR_control_cis_matrix_eqtl.R
#
#  $proj/Scripts/eqtls/cis/gtex/FDR_control_cis_matrix_eqtl.R
# 
...
# A few notes on FDR calculation
# The Benjamini-Hochberg procedure only runs up to the point where we have p-values (0.01 in the case of cis)
# We actually don't simulate p-values beyond that in order to get the full results
# The reason for this is that if we assume that the p-values are roughly uniformly distributed past our threshold,
# then the FDR will increase monotonically.
# Something we might be able to do in the future is incorporate the empirical p-values (empPvals)
# from the set of null test stats we have from the permuted runs. For now we don't incorporate them because
# the p-values from these null runs are roughly uniformly distributed (as can be seen from p-value histograms)
...
original_dir = paste0(proj_dir, '/Output/cis-mapping/gtex/MatrixEQTL/gct-normalized-1M/')
tissue_name = 'thyroid'
write_dir = paste0(proj_dir, '/Output/cis-mapping/gtex/cis-eQTLs/MatrixEQTL/gct-normalized-1M/')
...
for (item in list_files) {
    ...
    cumulative_me_cis_total[[i]] = me_cis$eqtls
    cumulative_me_cis_permute_total[[i]] = me_cis_permute$eqtls

    all_hist_vals_cis_total = all_hist_vals_cis_total + me_cis$hist.counts
    all_hist_vals_cis_permute_total = all_hist_vals_cis_permute_total + me_cis_permute$hist.counts

    all_n_tests_cis_total = all_n_tests_cis_total + me_cis$ntests
    all_n_tests_cis_permute_total = all_n_tests_cis_permute_total + me_cis_permute$ntests
