##################################################
#  extract_gtex_gct_normalized_expression.py
#
#  $proj/Scripts/misc/gtex/extract_gtex_gct_normalized_expression.py
# 
...
# First process the normalized files
# Note: the initial files were not formatted correctly, since the .bed files were missing the .gz extension

# Read in the gz files and the corresponding tissue names, list of subjects with genotypes and nonoverlapping gene list
...
v6p_consortium_gene_table = pd.read_csv(proj_dir + '/Data/Resources/annotations/gtex.v6p.genes.with.without.cis.eqtls.tested.txt', sep = '\t')
v6p_consortium_gene_list = np.array(v6p_consortium_gene_table['GENE'])
...
for files in bedfiles:
	...
	tissue_df_auto = tissue_df_auto[tissue_subjects]
	# Take only from the list of nonoverlapping genes
	tissue_df_auto_all = tissue_df_auto.loc[[x for x in v6p_consortium_gene_list if x in tissue_df_auto.index]]
	# output files
	tissue_df_auto_all.to_csv(out_dir + tissue_name + '_v6p_consortium_autosomes_normalized.txt', sep = '\t')
