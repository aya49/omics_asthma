...
#------------
sampleMap0 <- read.csv(paste0(data_dir, "/data/validationStudy/reCalibration/attenuation/AttenuationnanoElementsDemo_labSheet.csv"))
...
demo1_dis$Allergen_cleanLabel <- as.character(demo1_dis$Allergen_cleanLabel)
...
## different response
flippers <- names(rowSums(table(as.character(demo0$NAME), demo0$calculated_Response) != 0)[rowSums(table(as.character(demo0$NAME), demo0$calculated_Response) != 0) == 2])
demo1_val_repeat <- demo0[demo0$NAME %in% flippers, ]
table(as.character(demo1_val_repeat$NAME), demo1_val_repeat$calculated_Response)
# remove subjects challenges with different allergens
demo1_val_repeatDifferentResponse <- demo1_val_repeat[!(as.character(demo1_val_repeat$NAME) %in% names(rowSums(table(as.character(demo1_val_repeat$NAME), as.character(demo1_val_repeat$Allergen_cleanLabel)) != 0)[rowSums(table(as.character(demo1_val_repeat$NAME), as.character(demo1_val_repeat$Allergen_cleanLabel)) != 0) == 2])), ]

## same response
demo_sameResponse1 <- subset(demo0, calculated_Response != "Control")
dropFlippers <- rownames(table(as.character(demo_sameResponse1$NAME), demo_sameResponse1$calculated_Response))[rowSums(table(as.character(demo_sameResponse1$NAME), demo_sameResponse1$calculated_Response) == 0) == 1]
demo_sameResponse2 <- demo_sameResponse1[demo_sameResponse1$NAME %in% dropFlippers, ]
demo1_val_repeatSameResponse <- demo_sameResponse2[demo_sameResponse2$NAME %in% rownames(table(as.character(demo_sameResponse2$NAME), demo_sameResponse2$calculated_Response))[rowSums(table(as.character(demo_sameResponse2$NAME), demo_sameResponse2$calculated_Response) > 1) == 1], ]
## remove subjects challenged with different allergens
name.allergen <- table(as.character(demo1_val_repeatSameResponse$NAME), as.character(demo1_val_repeatSameResponse$Allergen_cleanLabel))
...
table(demo1_val_repeatDifferentResponse$calculated_Response, as.character(demo1_val_repeatDifferentResponse$Allergen_cleanLabel))
table(demo1_val_repeatDR$calculated_Response, as.character(demo1_val_repeatDR$Allergen_cleanLabel),
...
eset_pos0 <- dat %>% filter(CodeClass == "Endogenous") %>% 
  spread(Name, posNorm)
eset_pos <- cpm(as.matrix(eset_pos0[, -c(1:2)]), log=T)
...
eset_pos_hk <- cpm(eset_pos_hk, log=T)
...
meta_fileraw = get(load(paste0(root,"/result/meta/file.raw.Rdata")))
meta_fileraw_id = paste0(meta_fileraw$id,"_",meta_fileraw$date,"_",meta_fileraw$time)

demo_ = demo0
demo_$NAME[grepl("WRF",demo_$NAME)] = "WRF"
demo_id = paste0(demo_$NAME,"_",demo_$AIC_YMD,"_",demo_$Time)
demo_ = demo_[match(meta_fileraw_id,demo_id),]
# demo_ = demo_[match(rownames(eset_pos_hk),rownames(demo_)),]
demo_ = demo_[!grepl("NA",rownames(demo_)),]

m <- eset_pos_hk[match(rownames(demo_),rownames(demo_)), ]
rownames(m) = demo_$NAME
save(m, file=paste0(root,"/result/feat/rnaelements.pre.Rdata"))
write.csv(m, file=paste0(root,"/result/feat/rnaelements.pre.csv"))


# meta_col
library(annotables)
grch38_dt <- merge(as.data.frame(grch38_tx2gene), as.data.frame(grch38), by="ensgene")
genes = sapply(strsplit(colnames(m),"_"), function(x) x[1])
genes[grepl("CECR1",genes)] = "ADA2"
genes[grepl("FAM65B",genes)] = "RIPOR2"
genes[grepl("CYTB",genes)] = "MT-CYB"
grep("unknown",genes)

gene_dt = grch38_dt[match(genes,grch38_dt$symbol),]
which(is.na(gene_dt$symbol)) # "noNamelncRNA" "unknown2a"    "unknown2"     "unknown1"     "CECR1"        "unknown3"     "FAM65B" "CYTB" 
gene_dt[is.na(gene_dt$symbol),c("start","end","chr")] = 0
gene_dt[is.na(gene_dt$symbol),c("symbol")] = genes[is.na(gene_dt$symbol)]

gene_dt$id = colnames(m)
save(gene_dt, file=paste0(root,"/result/meta/col-rnaelements.Rdata"))
write.csv(gene_dt, file=paste0(root,"/result/meta/col-rnaelements.csv"))
```
