...
## input directory
data_dir = paste0(root, "/data/metab")

## output directory
feat_dir = paste0(result_dir,"/feat/metab")
...
```{r}
## demographics
demo <- readRDS(paste0(data_dir,"/data/allsitesDemo.rds"))
...
#
##===================================
plate.96wells <- read.csv(paste0(data_dir, "/data/2015-02-10_Conc_asthmaDiscoveryPlateFeb10.2015.csv"), stringsAsFactors=FALSE)
...
meta$id[grepl("WRF",plate_id)] = "WRF"
...
metNamesDat <- read.csv(paste0(data_dir, "/data/metNames.csv"), row.names = 1)
...
save(esetAsthmaPre_, file=paste0(feat_dir,".pre.Rdata"))
save(esetAsthmaPost_, file=paste0(feat_dir,".post.Rdata"))
save(esetAsthmaDiff_, file=paste0(feat_dir,".diff.Rdata"))
write.csv(esetAsthmaPre_, file=paste0(feat_dir,".pre.csv"))
write.csv(esetAsthmaPost_, file=paste0(feat_dir,".post.csv"))
write.csv(esetAsthmaDiff_, file=paste0(feat_dir,".diff.csv"))
...
pdf(paste0(data_dir, "/results/Figure1A.pdf"), width = 8, height = 4.5)
...
```{r}
jpeg(paste0(data_dir, "/results/Figure1B.jpeg"), width = 600, height = 400)
...
pdf(paste0(data_dir, "/results/Figure1C.pdf"), width = 6, height = 6)
deGen
dev.off()
...
pdf(paste0(data_dir, "/results/FigureE1.pdf"), width = 5, height = 5)
...
NMF::aheatmap(fc, annCol = list(Type = metLabels),
  annColors = "Set2",
  filename = paste0(data_dir, "/results/Figure1D.pdf"), 
  border = TRUE, width = 7, height = 7,
  main = "")


pdf(paste0(data_dir, "/results/Figure1Da.pdf"), width = 4, height = 4)
