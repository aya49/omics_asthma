---
title: "A working tutorial for modeling protein expression by using limma package"
author: "Ozan Aygun"
date: "May 31, 2017"
output:
  html_document:
...
eBayes() takes the model object returned by lmFit() and performs empirical Bayes moderation. This involves moderating the standard errors towards a common value obtained from the entire data set. This is important for making our inferences robust given the number of independent replicates (biological replicates) in the experiments are often very low (usually 2 for proteomics). eBayes also computes moderated versions of t and F statistics to evaluate differential expression.
...
For one sample comparisons (essentially one-sample Moderated t-test), it is more intuitive to feed a pure data matrix into the lmFit that exclusively contains the biological replicates of a particular **class**. (Class is a treatment category, which is represented by biological replicates.)
...
We use the model returned from lmFit to perform empirical Bayes moderation:
...
Note that the order of the columns in the data.matrix are critical. They represent the individual biological replicates of the groups. We need to construct the design matrix to properly match each of the replicate to its respective class.
...
- Note that the coefficients for K48 condition (Which is actually K48 vs Mono) are the same as logFC.
...
2. Then making parameters robust by applying empirical Bayes shrinkage method (because we have only few independent measurements for each gene (or model), thus we utilize the variability from the entire data set to make our standard error estimates more robust).
...
For this example, let's include a 3rd group into the data.matrix, which is K63. Therefore, we will have the following biological replicates for each group:
...
- a p.value cut off for the null hypothesis test B1 = 0 for each gene, for each contrast. Defaults is 0.05.
...
Another neat function limma provides is the **vennDiagram()**, which computes classification counts and draws a Venn diagram. We pass the results object created by the decideTests() function. We can also specify additional aesthetic arguments:
...
The distribution of F-test p-values suggest that essentially not many proteins look like significantly varying across the experiments.
...
**Note that we are interested in whether K48 or K63 treatments are changing any genes beyond the mono, and whether this reponse is different in Mutant cells compared to Wild-type.**

## Define questions of interest

Questions of interest always come first. This is particularly critical for factorial design, since the final model will be quite complex. We need to set up the questions upfront in order to fit and test the relevant contrasts from our model.

In this experiment, the relevant questions could be:

1. Which genes respond to a given treatment in Wt cells? 
2. Which genes respond to a given treatment in Mutant cells?
3. Which genes **respond differently** to a given treatment in mutant compared to wild-type cells?  

## Prepare the data.column - experiment.class match table (Annotation table or Target matrix)

This is a table we need to prepare to clearly tabulate the relevant data columns in the data matrix, with respect to the classes they belong to. This can be prepared in an R environment or externally using excel and then imported into the R session like I did here.
...
The table is intuitively constructed. In limma, this table is also called as "target matrix", this is the main matrix we use to construct the design matrix.

From this target matrix we can understand that there are 12 samples in total. Each Strain class is represented by 3 treatments and each of them have 2 **independent** biological replicates.

This **independence** is important because if we had resampled from the same individual (Wild-type or Mutant), we had to take into account the correlation between the observations. This would necessitate using mixed effect models. 

## Set the factor levels correctly to define reference levels

It is especially important to define the factor levels correctly based on the questions we are interested in. Let's recall the overall objective and our questions. This time try to write these questions in the form of their relations with the factor classes:

**We are interested in whether K48 or K63 treatments are changing any genes beyond the mono, and whether this reponse is different in Mutant cells compared to Wild-type.**

1. Which genes respond to a given treatment in Wt cells? (**Relates to: WT.K48 vs WT.mono, WT.K63 vs WT.mono, two seperate comparisons**)
2. Which genes respond to a given treatment in Mutant cells? (**Relates to: Mut.K48 vs Mut.mono, Mut.K63 vs Mut.mono comparisons, two seperate comparisons**)
3. Which genes **respond differently** to a given treatment in mutant compared to wild-type cells? (**Relates to the difference of differences**: 

- For K48 treatment: [Mut.K48 - Mut.mono] - [WT.K48 - WT.mono]
- For K63 treatment: [Mut.K63 - Mut.mono] - [WT.K63 - WT.mono]

Both of these contrasts will be represented as **interaction terms** in our model.

 )  
 
 Therefore, our reference levels for two factors are:
 
 - Strain: Wt
 - Treatment: Mono

```{r}
annotations$Treatment <- factor(annotations$Treatment, levels = c("Mono","K63","K48"))
annotations$Strain <- factor(annotations$Strain, levels = c("Wt","Mut"))
```

## Prepare the data matrix

It is not difficult to extract the relevant data columns since we already know them from the target matrix:

```{r}
d <- data.mat[,which(colnames(data.mat) %in% annotations$Column.Name)]
```

Note that the gene IDs are already attached as row names in the data. Otherwise, we would be attaching them.

Make sure the order of the columns in the data matrix is the same as the order in the annotation (target) matrix. Otherwise, we need to make sure there are in the same order:

```{r}
identical(colnames(d),as.character(annotations$Column.Name))
```

Great! Out data matrix is ready for modeling.

## Prepare the design matrix

We previously used model.matrix() function for fitting two or multiple group comparisons. Here the formula interface is a little different:

```{r}
design <- model.matrix( ~ Treatment * Strain, data = annotations)
design
```

As we notice from the design matrix we have:

- main effects of each treatment (except the baseline treatment)
- main effect of the mutant strain (not the reference Wt strain)
- interaction terms of K63 and K48 treatments with the mutant strain
- intercept

Note that the intercept represents the mean logFC for a gene in the both reference categories, which is TreatmentMono.StrainWt. All other coefficients are relative to this term.

**Before going into the details of the model, we need one edit in the column names. The colon (:) in the interaction term names needs to be replaced. This is because the downstream function make.contrasts() can not handle these types of special charecters in column names.**

We replace ":" by "." in the column names of the design matrix:

```{r}
colnames(design) <- sapply(colnames(design), function(x){gsub(":",".",x)})
design
```

We now have our design matrix ready.

## Fit the overall model using lmFit()

This is straightforward since we both have the data matrix and the design matrix prepared:

```{r}
fit.factorial <- lmFit(d, design = design)
```

## Write the model equation and understand interpret the coefficients

As always, writing down the equation is critical. Here we have the following model:

Log Exp Ratio = B0 + B1 * K63 + B2 * K48
                   + B3 * Mut
                   + B4 * K63.Mut + B5 * K48.Mut
                   
where the **invisible** categorical variables are:

- K63 = 1 when the treatment is K63 and 0 otherwise
- K48 = 1 when the treatment is K48 and 0 otherwise
- Mut = 1 when the strain is Mutant and 0 if it is Wild-type
                   
In our encoded model (design matrix) the coefficients are nicknamed as:

- B0: (Intercept)
- B1: TreatmentK63
- B2: TreatmentK48
- B3: StrainMut
- B4: TreatmentK63.StrainMut
- B5: TreatmentK48.StrainMut

These coefficients have their own interpretations:

- **B0: (Intercept):** mean logFC in the TreatmentMono and StrainWt 
- **B1: TreatmentK63:** mean logFC difference of [TreatmentK63 - TreatmentMono] in StrainWt (i.e: TreatmentK63 effect in the Wt strain)  
- **B2: TreatmentK48:** mean logFC difference of [TreatmentK48 - TreatmentMono] in StrainWt (i.e: TreatmentK48 effect in the Wt strain)
- **B3: StrainMut:** mean logFC difference of [StrainMut - StrainWt] under TreatmentMono condition (i.e: strain effect at the baseline treatment level)
- **B4: TreatmentK63.StrainMut:** mean logFC difference of ([TreatmentK63 - TreatmentMono] in  StrainMut) - ([TreatmentK63 - TreatmentMono] in  StrainWt)
- **B5: TreatmentK48.StrainMut:** mean logFC difference of ([TreatmentK48 - TreatmentMono] in  StrainMut) - ([TreatmentK48 - TreatmentMono] in  StrainWt)

Note that not all of the questions we are interested in are directly available from the model coefficients. This doesn't mean we can not construct the contrasts we are interested in. We will derive these contrasts to fit exactly the questions we are interested in. The model coefficients and their interpretation we already described will become handy during these operations.

## Translate questions into null hypotheses, then into desired contrasts in model terms

This is the critical part. We need to recall our questions and **associated null hypotheses(H0)** to fit construct the relevant contrasts.

Remember our model and categorical variables:
...
- H0: Mean.WT.K48 - Mean.WT.mono = 0 ,or we can write: Mean.Wt.K48.vs.Mono = 0
- H0: Mean.WT.K63 - Mean.WT.mono = 0 ,or we can write: Mean.Wt.K63.vs.Mono = 0

Using the coefficients, these differences are represented as:

- Mean.Wt.K48.vs.Mono = B0 + B2 - B0 = B2
- Mean.Wt.K63.vs.Mono = B0 + B1 - B0 = B1

In terms of the nicknames in our model, we can re-write:

___

- Mean.Wt.K48.vs.Mono = TreatmentK48
- Mean.Wt.K63.vs.Mono = TreatmentK63

___

These will be the basis for our relevant contrasts.
...
- H0: Mean.Mut.K48 - Mean.Mut.mono = 0, or we can write: Mean.Mut.K48.vs.Mono = 0
- H0: Mean.Mut.K63 - Mean.Mut.mono = 0, or we can write: Mean.Mut.K63.vs.Mono = 0

Using the coefficients, these differences are represented as:

-  Mean.Mut.K48.vs.Mono: B0 + B2 + B3 + B5 - (B0 + B3) = B2 + B5
-  Mean.Wt.K63.vs.Mono: B0 + B1 + B3 + B4 - (B0 + B3) = B1 + B4

In terms of the nicknames in our model, we can re-write:

___

- Mean.Mut.K48.vs.Mono =  TreatmentK48 + TreatmentK48.StrainMut
- Mean.Mut.K63.vs.Mono =  TreatmentK63 + TreatmentK63.StrainMut
...
**Relates to the difference of differences**: 

i.e: For which genes the treatment effects are significantly modulated by the strain type?
...
- H0: [Mean.Mut.K48 - Mean.Mut.mono] - [Mean.Wt.K48 - Mean.Wt.mono] = 0
- H0: [Mean.Mut.K63 - Mean.Mut.mono] - [Mean.Wt.K63 - Mean.Wt.mono] = 0

or, we can write:

- Mean.Mut.K48.vs.Mono - Mean.Wt.K48.vs.Mono = Mean.K48.Mut.vs.Wt
- Mean.Mut.K63.vs.Mono - Mean.Wt.K63.vs.Mono = Mean.K63.Mut.vs.Wt

Using the coefficients, these differences are represented as:

- Mean.K48.Mut.vs.Wt = B2 + B5 - B2 = B5
- Mean.K63.Mut.vs.Wt = B1 + B4 - B1 = B4

In terms of the nicknames in our model, we can re-write:

___

- Mean.K48.Mut.vs.Wt = TreatmentK48.StrainMut
- Mean.K63.Mut.vs.Wt = TreatmentK63.StrainMut 

___

Note that both of these contrasts are represented as **interaction terms** in our model!


## Make the contrasts using makeContrasts()

That was the hard work! The rest is more straightforward. What we need to to do is to define these contrasts in the way that R can understand and prepare from the design matrix we prepared.

For this purpose, we will use the makeContrasts() function we already used before. Recall that this function essentially needs two arguments; 

1. A list of contrasts (**in fitted model terms(or nicknames)**)
2. levels = design matrix

Here, we are essentially using this function in the same way. The only difference is that we are going to use **named** contrasts to explicitly define the contrasts inside the function.

**Another important fact to notice about makeContrasts() is the flexible way we define the contrasts inside the function using arithmetic operators. This provides a handy way to virtually engineer various possible combinations of contrasts that might not be immediately available as model coefficients to us. This becomes more critical as the model complexity increases.**

```{r}
contrasts <- makeContrasts(
  # Which genes respond to a given treatment in Wt cells?
  Mean.Wt.K48.vs.Mono = TreatmentK48,
  Mean.Wt.K63.vs.Mono = TreatmentK63,
  
  # Which genes respond to a given treatment in Mut cells?
  Mean.Mut.K48.vs.Mono =  TreatmentK48 + TreatmentK48.StrainMut,
  Mean.Mut.K63.vs.Mono =  TreatmentK63 + TreatmentK63.StrainMut,
  
  # Which genes respond differently to a given treatment in mutant compared to wild-type cells?
  Mean.K48.Mut.vs.Wt = TreatmentK48.StrainMut,
  Mean.K63.Mut.vs.Wt = TreatmentK63.StrainMut 
  
, levels = design)

rownames (contrasts)[1] <- '(Intercept)'
contrasts
```

Notice that we had to rename the first rowname since makeContrasts stripped of the '( )' in the original name.

Therefore, we have explicitly prepared the 6 contrasts we are interested in. In the subsequent stages, we will fit these contrasts and obtain estimates for them. In each case, we will also obtain an adjusted p-value that tests the null hypothesis associated with each contrast we described above.

## Get overall model estimates and p-value

This is an optional step but could be useful to get the estimates for the overall model fit for each gene:

```{r}
fit.overall <- eBayes(fit = fit.factorial, robust = TRUE)
overall.results <- toptable(fit.overall, number = nrow(fit.overall), adjust.method = "fdr")
head(overall.results)
```

Note that this overall model results do not mean much about our specific hypotheses.

## Using contrasts.fit() to fit model for the comparisons in factorial design

This step is no longer a new theme for us. We are simply going to use the contrasts.fit() function to fit the model for the specific contrasts we defined above.

```{r}
fitted.contrasts <- contrasts.fit(fit = fit.factorial, contrasts = contrasts)
fitted.contrasts <- eBayes(fitted.contrasts)
```

## Examine the coefficients and p-values

Let's examine the coefficients estimated by our model fit:

```{r}
head(coef(fitted.contrasts))
```

Note that we have obtained an estimate for each contrast we defined. 

These estimates indeed correspond to the differences we defined. For instance the coefficients returned for the Mean.Wt.K48.vs.Mono contrast are simply the logFC we obtained in our simple-two sample comparison previously (see 3.4). Therefore, it is nice to see that our model is built up on and is able to extract those individual comparisons we had investigated previously.

Since we have 6 contrasts in this case, we have 6 coefficients (estimates) and therefore, 6 p-values, and 6-sets of inferences (results) tables. 

For each contrast, we can examine these results separately. 

For instance, 

### Examine if the p-values from the Mean.Wt.K48.vs.Mono contrast are the same as the ones we obtained in the simple two group comparisons

Note that Mean.Wt.K48.vs.Mono is the first coefficient in the contrasts matrix, so we will extract the respective results using the topTable:

```{r}
results.Mean.Wt.K48.vs.Mono <- toptable(fitted.contrasts, coef = 1,number = nrow(d) , adjust.method = "fdr", sort.by = "none", confint = TRUE)

head(results.Mean.Wt.K48.vs.Mono)
head(two.group.significance)
```

Great! Note that even though the estimates are the same as expected, the p-values and confidence intervals are changed. This is because the inferences are made in the context of the entire regression model and effects are adjusted to other covariates fitted in the same model. 

We notice that overall p -values are getting slightly lower for each gene in the context of the large model, however adjusted p-values imply the same inference (i.e: failed to reject H0)

### Examine which genes respond differently to K48 treatment in Mutant v.s Wild-type cells?

Recall that we refined this question into a single coefficient, which is the interaction term between K48 and Mut in our model. The nickname of the contrast is:  **Mean.K48.Mut.vs.Wt**

Note that this is the 5th contrast in the fitted.contrasts object:

```{r, fig.width= 8}
results.Mean.K48.Mut.vs.Wt <- topTable(fit = fitted.contrasts, coef = 5, adjust.method = "fdr", sort.by = "none", confint = TRUE, number = nrow(d))
head(results.Mean.K48.Mut.vs.Wt)
hist(results.Mean.K48.Mut.vs.Wt$P.Value, col = "navy", breaks = 100)
```

The overall distribution of p-values suggest that it is unlikely any genes display change based on this contrast.

### Combine the model results into a single table

We can keep investigating the individual contrasts one-by-one, or we can combine them into a single results table where each row corresponds to a gene and each column corresponds to a results parameter for a given contrast. Since result outputs are generic, we need to append them with contrast names to distinguish in the final results table.

I wrote a function that could be useful for this purpose. 

```{r}
parseResults <- function(fit,contrasts){
        # fit: contrast model fit object generated by lmFit() and then contrasts.fit()
        # contrasts: the contrasts matrix used to generate the model, containing desired contrasts
        require(limma)
        contrasts <- data.frame(contrasts)
        n <- nrow(fit$coefficients)
        results <- data.frame(id = row.names(fit$coefficients))
        for(i in seq_along(names(contrasts))){
             temp.results <- topTable(fit = fit, coef = i, confint = FALSE, sort.by = "none",
                                      adjust.method = "fdr", number = n)
             colnames(temp.results) <- paste(colnames(temp.results),colnames(contrasts[i]), sep = ".")
             results <- cbind(results,temp.results)
        }
        return(results)
...
combined.results.factorial <-  parseResults(fitted.contrasts,design)

```

# Time-course experiments

These experiments can be handled just like the TWO-WAY factorial design analysis we completed in section 5. In this case the assumption is that the samples are independent (i.e: the expression values are not repeated measurements obtained from the same sample over the time.). 

The design and analysis then becomes very similar to the factorial analysis. For instance, we would investigate the impact of a treatment at 0, 6 and 12 hours in Wt and Mutant cells. This would give us 2 x 3 categories, where we can fit the models and contrasts as we performed in Section 5.

In the case of repeated measures, in other words, when the assumption of independence is violated, a longitudinal analysis method will be required, which involves fitting more complex models using an appropriate covariance matrix structure. 