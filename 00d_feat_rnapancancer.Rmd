...
subtitle: "input: nanostring pancancer data; output: well formatted data and meta col files"
output:
  html_document:
    theme: paper
    highlight: zenburn
...
libr(append(pkgs(),c("affy","GGally","ggbiplot","NormqPCR","tufte")))
...
#----------------------------------------------
setName <- paste("Set", 1:length(rnapc_data_dir))
date <- c("2015-07-03","2015-07-03","2015-07-03")
DatList <- list()
for(h in 1 : length(rnapc_data_dir)){
  files <- list.files(rnapc_data_dir[h], full.names = TRUE)
...
pdf(rnapc_pairs_dir, height = 8, width = 8)
...
ann0 <- read.csv(cell_pc_immune_dir, row.names = 1)
