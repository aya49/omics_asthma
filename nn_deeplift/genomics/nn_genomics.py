# simdna package is needed for reading the data; install it if it doesn't exist
try:
    import simdna
...
# convert to deeplift model (with 3 different variants: rescale on the conv layers and revealcancel on the fully-connected layers (the genomics default), rescale on all layers, and revealcancel on all layers)
...
# to ensure that the conversion happend correctly, make sure predictions are the same as the original model
...
# we propose averaging the scores produced using mutliple references
# which are produced by shuffling the original sequence.
# We find in practice that this can give more robust results.
